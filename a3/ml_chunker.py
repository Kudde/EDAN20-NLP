...
"""
__author__ = "Pierre Nugues, Ludde and Kev"
...
def predict(test_sentences, feature_names, f_out, y):
    """
    ...
    for test_sentence in test_sentences:
        ...
        for x in X_test_dict:
            ...
            predicted_symbols += [y_test_predicted[0]]

            print('predicted')
            print(predicted_symbols)

            c_n2 = c_n1
            c_n1 = y_test_predicted[0]
            print("c_n1 : " + c_n1)

        print("--------------------------")
        rows = test_sentence.splitlines()
        rows = [rows[i] + ' ' + predicted_symbols[i] for i in range(len(rows))]
        for row in rows:
            f_out.write(row + '\n')
        f_out.write('\n')
    f_out.close()
...
if __name__ == '__main__':
    ...
    # classifier = linear_model.Perceptron(tol=1e-3, random_state=0)
    ...
    f_out = open('out', 'w')
    predict(test_sentences, feature_names, f_out, y)
