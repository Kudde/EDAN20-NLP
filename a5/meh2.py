...
def extract_f(file, feature_names):
...
def parse(feature_names, sentences, model, vectorizer):
    column_names_2006 = ['id', 'form', 'lemma', 'cpostag', 'postag', 'feats', 'head', 'deprel', 'phead', 'pdeprel']
    X = []
    y = []
    for sentence in sentences:
        ...
        while queue:
            features = extract(stack, queue, graph, feature_names, sentence)
            trans_nr = model.predict(vectorizer.transform(features))
...
if __name__ == '__main__':
    train_corpus = './train.conll'
    test_corpus = './test.conll'
    ...
    feature_names3 = ["stack0_POS", "stack0_word", "stack1_POS", "stack1_word",
                      ...
                      "next_word_POS", "next_word_word", "can-re", "can-la"]
    ...
    x_train, y_train = extract_f(train_file, fn)
    x_test, y_test = extract_f(test_file, fn)
    ...
    sentences = conll.read_sentences(test_corpus)
    test_sentences = conll.split_rows(sentences, column_names_2006)

    # -------------- A6

    print("Parsing")
    parse(fn, test_sentences, model, vec)
    print("K. DOne. Bye")
